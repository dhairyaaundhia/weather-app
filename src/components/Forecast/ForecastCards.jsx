  ...
  return (
    ...
    >
      {forecast?.forecastday?.slice(0, 4).map((day, index) => (  
        <div
          key={day.date_epoch}
          className={`border-[3px] bg-[#282c34] hover:bg-slate-100 dark:hover:bg-slate-600 dark:bg-slate-700 h-44 w-full grid grid-cols-4 grid-rows-2 justify-center items-center p-6 cursor-pointer ${
            active === index
        ...
        >
          <div className='col-span-2'>
            <h3 className='text-xl dark:text-gray-100'>
              {DAY[(new Date(day.date).getDay() + 1) % 7]} {/* Ensure correct wrapping */}
            </h3>
